...
private:
		...
		void flush() {
			FD i = index, d = data;
			size_t indexHeadLen;
			lseek(i.getfd(), BeginOfIndex-3*sizeof(size_t), SEEK_SET);
		...
		void heapflush() {
			...
			off_t oldPlace;
			Read(r.getfd(), &oldPlace, sizeof(off_t));
			lseek(r.getfd(), oldPlace, SEEK_SET);
			size_t len;
			Read(r.getfd(), &len, sizeof(len));
			push(EmptySpace(oldPlace, len*sizeof(dump)+sizeof(size_t)));
			...
			lseek(r.getfd(), sizeof(off_t), SEEK_SET);
			Write(r.getfd(), &newPlace, sizeof(size_t));
		}
